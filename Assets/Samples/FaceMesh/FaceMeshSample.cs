...
public sealed class FaceMeshSample : MonoBehaviour
{
    ...
    [SerializeField]
    bool useLandmarkToDetection = true;

    [SerializeField]
    RawImage cameraView = null;

    [SerializeField]
    RawImage croppedView = null;
    ...
    void Start()
    {
        string detectionPath =
            Path.Combine(Application.streamingAssetsPath, faceModelFile);
        faceDetect = new FaceDetect(detectionPath);

        string faceMeshPath =
            Path.Combine(Application.streamingAssetsPath, faceMeshModelFile);
        faceMesh = new FaceMesh(faceMeshPath);

        string cameraName =
            WebCamUtil
                .FindName(new WebCamUtil.PreferSpec()
                { isFrontFacing = true, kind = WebCamKind.WideAngle });
    ...
    void DrawResults(FaceDetect.Result detection, FaceMesh.Result face)
    {
        ...
        // Draw Face Detection
        {
            ...
            {
                draw
                    .Point(MathTF.Lerp(min, max, new Vector3(p.x, 1f - p.y, 0)),
                    0.1f);
            }
        }
        draw.Apply();
